...
const EditAllocationModal: React.FC<IAllocation> = (data) => {
    ...
    const { setEditAllocationModalOpen, setAllocationCreated } = useContext(AppContext);
    const [title, setTitle] = useState<string>("");
    const [amount, setAmount] = useState<number>(0);
    const [allocation, setAllocation] = useState<string>();
    const [mintDate, setMintDate] = useState<number | undefined>(data.mint_date);
    ...
    const initAction = () => {
        ...
        setRole(data.role as number);
        setServerID(data.for_server as string);
        setMintDate(data.mint_date);
        setTitle(data.title as string);
        setAmount(data.allocation as number);
        setMintHoldDays(data.vesting?.mint_hold_days as number);
        setContract(data.contract as string);
        setSecondaryBuyAmount(data.vesting?.secondary_buy_amount as number);
        setSecondaryBuyHoldDays(data.vesting?.secondary_buy_hold_days as number);
        setSecondaryBuyHoldHours(data.vesting?.secondary_buy_hours as number);
        setPriceVoid(data.vesting?.price_void as number);
    ...
    const handleSubmit = async () => {
        ...
        const data: any = {id, serverID:serverID,role, contract, allocation, mintHoldDays, secondaryBuyHoldDays, secondaryBuyHoldHours, secondaryBuyAmount, priceVoid, mintDate, title, amount };
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="grid gap-3">
                <div className="flex flex-col gap-2">
                    ...
                    <input type="datetime-local" className="outline-none placeholder:text-sm placeholder:font-normal px-3 py-[10px] rounded-md bg-cdark-50 border border-cgrey-200 text-cwhite"   value={mintDate ? moment(mintDate * 1000).format("YYYY-MM-DDTHH:mm") : ""} onChange={(e) => handleMintDateTime(e)} />
                </div>
            </div>
            <div className="grid gap-3">
                {/* <div className="flex flex-col gap-2">
